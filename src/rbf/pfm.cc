...
bool incrementCounter(FILE* file, int mode){
	// Invalid value of mode, return error. The values of mode can only be: 1-read, 2-write, 3-append.
...
bool createHiddenPage(FILE* file){
	...
	memcpy(((char*)hiddenPage)+offset, &cnt, sizeof(int));		// Initially setting read counter value to zero. (byte 4 to byte 8 in hidden page)
	offset += sizeof(int);

	memcpy(((char*)hiddenPage)+offset, &cnt, sizeof(int));		// Initially setting write counter value to zero. (byte 8 to byte 12 in hidden page)
	offset += sizeof(int);

	memcpy(((char*)hiddenPage)+offset, &cnt, sizeof(int));	// Initially setting append counter value to zero. (byte 12 to byte 16 in hidden page)
	offset += sizeof(int);
